...
public class ConfigurationSwitch<T> extends Switch<T> {
	...
	protected T doSwitch(int classifierID, EObject theEObject) {
		switch (classifierID) {
			case ConfigurationPackage.CONFIG_MAP: {
				ConfigMap configMap = (ConfigMap)theEObject;
				T result = caseConfigMap(configMap);
				if (result == null) result = caseResource(configMap);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
			case ConfigurationPackage.SECRET: {
				Secret secret = (Secret)theEObject;
				T result = caseSecret(secret);
				if (result == null) result = caseResource(secret);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
